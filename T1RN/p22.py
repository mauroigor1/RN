...
# paso de ytrain a matriz para trabajar
...
#Fit the model
history=model.fit(X, Y, batch_size=300, epochs=25,validation_split=0.3, verbose=1)

scores = model.evaluate(Xt, Yt)   
print("\n%s: %.2f%%" % (model.metrics_names[1], scores[1]*100))#precisión
output = K.function([model.layers[0].input], [model.layers[2].output])
output = output([X])[0]
